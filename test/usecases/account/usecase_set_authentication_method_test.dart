...
void main() {
  ...
  test('biometrics performs correct calls', () async {
    when(repository.saveAuthenticationMethod(any))
        .thenAnswer((_) => Future.value(null));

    await setAuthenticationMethodUseCase.biometrics();

    final authMethod = BiometricAuthentication();
    verify(repository.saveAuthenticationMethod(authMethod)).called(1);
  });
  ...
  test('password performs the correct calls', () async {
    ...
    await setAuthenticationMethodUseCase.password("password");
